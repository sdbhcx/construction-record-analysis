...
* **挂起恢复**：当任务因超时、死信重试或人工介入而中断时，系统通过 Checkpointer 持久化保存 Agent 的中间状态、工具调用轨迹和校验失败上下文；前端恢复面板在用户补全信息后重新注入相同任务上下文，使状态机从挂起节点继续执行。
...
* **Redis + Celery (中间件/调度)**：Redis 用作 Celery 的 Broker 和 Result Backend，同时提供分布式锁和全局并发计数器，确保提交给 vLLM 的高并发请求处于 GPU 显存的安全水位之内；对于需要断点续传的 Agent 任务，可配合持久化 Checkpointer 保存挂起快照与恢复 token，供 DLQ 重放和人工恢复面板使用。
* **大小模型级联推理服务 (推理引擎)**：
